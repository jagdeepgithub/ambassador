...
var (
	...
	defInstallationMethodsInfo = []struct {
		Method   int
		Label    string
		Name     string
		LongName string
...
// getInstMethodName return the string-representation of the installation method
func getInstMethodName(method int) (string, string) {
	for _, info := range defInstallationMethodsInfo {
		if info.Method == method {
			return info.Name, info.LongName
		}
	}
	return "", ""
}
...
func (i *Installer) GrabAESInstallID() error {
	...
	i.log.Print("> aesImage = %s", aesImage)
	podName := ""
	containerName := ""
...
func (i *Installer) GetExistingInstallation() (string, int, error) {
	...
	for _, info := range defInstallationMethodsInfo {
		...
		version, err := findForLabel(info.Label)
		if err != nil {
			continue // ignore errors
		}
		if version != "" {
			return version, info.Method, nil
		}
	}
	return "", instNone, nil
}
...
func (i *Installer) Perform(kcontext string) Result {
	...
	// namespace.
	installedVersion, installedMethod, err := i.GetExistingInstallation()
	if err != nil {
	...
	if installedVersion != "" {
		installedMethodName, installedMethodLongName := getInstMethodName(installedMethod)

		i.SetMetadatum("Cluster Info", "managed", installedMethodName)
		i.ShowAESExistingVersion(installedVersion, installedMethodLongName)
		i.Report("deploy", ScoutMeta{"already_installed", true})

		switch installedMethod {
		case instOSS, instAES, instEdgectl, instOperator:
			return i.ExistingInstallationFoundError(
				errors.Errorf("Existing AES %s found when installing AES %s",
					installedVersion, i.version), installedVersion)
		case instHelm:
	...
	if installedMethod == instHelm {
		...
		if installedVersion != i.version {
			err := errors.New(fmt.Sprintf("Helm version was %q, but latest version is %q", installedVersion, i.version))
			return i.ExistingInstallationFoundError(err, installedVersion)
		}
	} else if installedMethod == instNone {
		...
		if i.clusterinfo.noLoadBalancer {
			// do not use LoadBalancer in this kind of environments: use a NodePort
			strvals.ParseInto("service.type=NodePort", chartValues)
		}
...
func (i *Installer) UpdateClusterInfo() error {
	...
	if clusterNodeLabels, err := i.CaptureKubectl("get node labels", "", "get", "no", "-Lkubernetes.io/hostname"); err == nil {
		if strings.Contains(clusterNodeLabels, "docker-desktop") {
			...
			i.clusterinfo.isLocal = true
			i.clusterinfo.noLoadBalancer = true
...
type clusterInfo struct {
	...
	// True if this cluster does not support LoadBalancer
	noLoadBalancer bool
}
